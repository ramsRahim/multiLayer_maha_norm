...
class ImageNetO(Dataset):
    """ImageNet-O: adversarially filtered OOD images for ImageNet classifiers."""
    def __init__(self, transform=None, loader=default_loader):
        import torchvision.datasets as tvd
        root = os.path.join(paths_config.dset_location_dict['ImageNet-O'], 'imagenet-o')
        self._inner = tvd.ImageFolder(root, transform=transform)
        self.transform = transform
        self.loader = loader
        self.classes = self._inner.classes
        self.class_to_idx = self._inner.class_to_idx
        self.samples = self._inner.samples
        self.__name__ = 'ImageNetO'
